    ...
    switch (node.type) {
        ...
        case 'ObjectExpression':
            const properties = node.properties
                ...
                .filter((prop) => !['AssignmentPattern', 'ObjectPattern', 'ArrayPattern'].includes(prop.value.type));
        ...
        case 'UpdateExpression':
            ...
            if (node.argument.type === 'MemberExpression') {
                ...
                const value = !node.argument.computed &&
        ...
        case 'AssignmentExpression':
            if (node.left.type === 'MemberExpression') {
                ...
                visit(node.right);
                const value = !node.left.computed && node.left.property.type === 'Identifier'
        ...
        case 'MemberExpression':
            ...
            visit(node.object);
            visit(node.property);
            const value = !node.computed && node.property.type === 'Identifier'
                ? node.property.name
                : node.computed &&
...
function extractPOG(node, options) {
...
function createNode(type, loop, value) {
...
function pog(functions, options) {
