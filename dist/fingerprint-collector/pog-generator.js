...
function is_typeof(node) {
    return (node.consequent.type === 'Literal' &&
...
function createNode(type, pogState, loop, value) {
    const id = pogState.currentId++;
    const node = type === 'branch'
...
function connect(to, pogState) {
    pogState.prevIds.forEach((from) => connectPair(from, to, pogState));
}
function connectPair(from, to, pogState) {
    ...
    switch (node.type) {
        ...
        case 'start':
            if (node.next)
                throw new Error('Start Node has already been connected');
            node.next = to;
            return;
    }
}
function connect_end(pogState) {
    connect(pogState.endId, pogState);
    pogState.prevIds = [];
}
function addemptyBranch(pogState) {
    const thenPrevIds = [];
    const elsePrevIds = [];
...
function addBranch(node, pogState) {
    visit(node, pogState);
    const truthy = [];
    const falsy = [];
    const top = [];
...
function addOp(operator, pogState) {
    if (PATH_CLONING) {
        pogState.prevIds.forEach((from) => {
            if (from[0].type === 'block') {
                if (from[0].op)
                    from[0].op?.push(operator);
                else
                    from[0].op = [operator];
    ...
    (0, estree_walker_1.walk)(node, {
        enter(child) {
            switch (child.type) {
                ...
                case 'Literal': {
                    const v = child.value;
                    const isFalsy = v === null || v === 0 || v === false || v === '' || v === 0n;
                    pogState.prevIds.forEach(([_, env]) => {
                ...
                case 'UpdateExpression':
                    ...
                    if (child.argument.type === 'MemberExpression') {
                        ...
                        const value = !child.argument.computed &&
                ...
                case 'AssignmentExpression':
                    ...
                    if (child.left.type === 'MemberExpression') {
                        ...
                        const value = !child.left.computed && child.left.property.type === 'Identifier'
                ...
                case 'MemberExpression':
                    ...
                    visit(child.property, pogState);
                    const value = !child.computed && child.property.type === 'Identifier'
                        ? child.property.name
                        : child.computed &&
                ...
                case 'NewExpression':
                    ...
                    child.arguments.forEach((arg) => visit(arg, pogState));
