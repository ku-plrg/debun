...
const intraduphashCache = {};
const intraduplibCache = {};
const evaluate = (uniqueHashes, options, method = "", url = "") => {
    const allWebHashes = {};
    const libInfos = JSON.parse(fs_1.default.readFileSync(path_1.default.join(__dirname, `./data/debun-hashes/all-libs${method}.json`), "utf-8"));
    const libHashes = JSON.parse(fs_1.default.readFileSync(path_1.default.join(__dirname, `./data/debun-hashes/all-hash${method}.json`), "utf-8"));
    if (Object.keys(uniqueHashes).length === 0) {
        uniqueHashes = allWebHashes[url];
    }
    const blacklist = (blacklistcache[WEB_BLACKLIST_THRESHOLD] = JSON.parse(fs_1.default.readFileSync(path_1.default.join(__dirname, `./data/debun-hashes/blacklist-${WEB_BLACKLIST_THRESHOLD}${method}.json`), "utf-8")));
    const intraDupHashes = (intraduphashCache[`${DUP_THRESHOLD}`] = JSON.parse(fs_1.default.readFileSync(path_1.default.join(__dirname, `./data/debun-hashes/dups-${DUP_THRESHOLD}-hash${method}.json`), "utf-8")));
    const intraDupLibs = (intraduplibCache[`${DUP_THRESHOLD}`] = JSON.parse(fs_1.default.readFileSync(path_1.default.join(__dirname, `./data/debun-hashes/dups-${DUP_THRESHOLD}-libs${method}.json`), "utf-8")));
    let totalMatches = {};
    let type3Matches = {};
    let type2Matches = {};
    if (!uniqueHashes)
            ...
            Object.entries(matches).forEach(([lIdx, vIdxes]) => {
                ...
                vIdxes.forEach(([start, end]) => {
                    for (let vIdx = start; vIdx <= end; vIdx++) {
                        ...
                        if (!totalMatches[lIdx][vIdx])
                            totalMatches[lIdx][vIdx] = 0;
                        totalMatches[lIdx][vIdx]++;
                        if (matchType === 3) {
                            if (!type3Matches[lIdx])
                                type3Matches[lIdx] = {};
                            if (!type3Matches[lIdx][vIdx])
                                type3Matches[lIdx][vIdx] = 0;
                            type3Matches[lIdx][vIdx]++;
                        }
                        if (matchType === 2) {
                            if (!type2Matches[lIdx])
                                type2Matches[lIdx] = {};
                            if (!type2Matches[lIdx][vIdx])
                                type2Matches[lIdx][vIdx] = 0;
    ...
    Object.entries(totalMatches).forEach(([lIdx, matches]) => {
        const lib = Object.entries(libInfos).find(([_, libInfo]) => libInfo.id.toString() === lIdx);
        if (!lib)
            return;
        let type3Versions = [];
        let type2Versions = [];
...
const determineType = (matches) => {
    if (Object.keys(matches).length === 1) {
        ...
        if (Object.values(matches)[0].length === 1) {
            ...
            if (vFrom === vTo)
                return 3; // type 3: only one version!
        }
        return 2;
    }
